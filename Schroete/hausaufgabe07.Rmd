...
```{r}
for(s in c(3,4,5)){
bestehen <- pnorm(13,mean=mu,sd=s,lower.tail=FALSE)
output <- paste("Bei einer Standardabweichung von",s,"werden", bestehen*100,"% mit der Note 'sehr gut' bestehen.")
print(output)
}
```
...
antwort_hier
Es bestehen leider mehr Studenten nicht.

Warum?

antwort_hier
Sehr gute Noten gehen von 13 bis 15. Durchfallen kann man schon an 5 bis 0. Die "Chance", eine schlechtere Note zu bekommen, ist demnach höher.
...
antwort_hier
Die Verteilung ist linksschief, da die linke Seite "beiter" ist, d.h. der Gipfel sich auf der rechten Seite befindet.
Aber ich habe bei anderen gelesen, dass sie rechtsschief sein soll?! Denk ich verkehrtherum? Mein Gipfel liegt definitiv rechts bei den guten Noten und ist breiter bei den schlechteren Noten - also ist es doch laut Beschreibung linksschief...
...
2. Wie viele Studenten bekommen zumindest 10 NP?
    ...
    `r pnorm(10,mean=mu,sd=3) * n` Studenten bekommen zumindest 10 Notenpunkte.
    zumindest 10?!

3. Wie viele Studenten bekommen weniger als 10 NP?

    `code_hier` Studenten bekommen weniger als 10 Notenpunkte.
    `r pnorm(9,sd=3,mean=mu) * n` Studenten bekommen weniger als 10 Notenpunkte.
    oder mit 10 statt 9?!

4. Wie viele Studenten bekommen weniger als 8 NP?

    `code_hier` Studenten bekommen weniger als 8 Notenpunkte.
    `r pnorm(7,sd=3,mean=mu) * n` Studenten bekommen weniger als 8 Notenpunkte.
    oder mit 8 statt 7?!
