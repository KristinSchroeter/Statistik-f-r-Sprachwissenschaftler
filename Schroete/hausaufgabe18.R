...
#   = 0.5*x2 + x2 
#   = 1.5*x2
# Das sind doch die Regressionskoeffizienten! 


# Wie sieht es aus, wenn wir beide gleichzeitig aufnehmen? Machen wir zuerst eine Grafik:
# (x1 wird horizontal geplottet, x2 vertikal und y als Größe des Punktes)
ggplot(linreg,aes(x=x1,y=x2)) + geom_point(aes(size=y))


# Wir führen zunächst eine Regression aus, wo sowohl x1 als auch x2 Prädiktor
...
# Bei linearen Regression müssen wir immer aufpassen, dass unsere Prädiktoren
...
# F-Werte an sowie auch die t-Werte für die einzelnen Prädiktoren. Glauben Sie, 
...
cor.test(pyreg$y, pyreg$x1, methode="spearman")

# CODE_HIER 
cor.test(pyreg$y, pyreg$x2, method="pearson")
cor.test(pyreg$y, pyreg$x2, methode="spearman")

# Welche Art von Korrelation ergibt am meisten Sinn bei diesen Daten?
...
# Welche Methode ergibt hier am meisten Sinn? Korrelationen oder Regression?

# Die Daten sind übrigens *nicht* linear. x1 besteht aus zehn zufälligen Zahlen
# zwischen [1,10] und x2 besteht aus zehn zufälligen Zahlen zwischen [1,20]. 
